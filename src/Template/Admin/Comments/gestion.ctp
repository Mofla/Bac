            ...
            <?php if ($this->request->session()->read('Auth.User.role_id') == 1 || $this->request->session()->read('Auth.User.id') == $comment->user_id): ?>
                <div class="media-right">
                    ...
                    <button id="<?= $comment->id ?>" class="btn btn-xs btn-primary btn-comment-edit"><span
                            class="glyphicon glyphicon-edit"></span></button>
                    <?= $this->Form->postLink('<span class="glyphicon glyphicon-trash"></span>', ['controller' => 'Comments', 'action' => 'delete', $comment->id], ['confirm' => 'Supprimer ce commentaire ?', 'class' => 'btn btn-xs btn-danger', 'escape' => false]) ?>
                </div>
            <?php endif; ?>
...
<script>
    ...
    $('.btn-comment-edit').unbind().bind('click', function () {
        ...
        $.ajax({
            type: "GET",
            url: "<?= $this->Url->build(['controller' => 'Comments', 'action' => 'edit','prefix' => false]) ?>",
            data: {id: id},
